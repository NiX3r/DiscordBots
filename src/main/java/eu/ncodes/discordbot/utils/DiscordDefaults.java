package eu.ncodes.discordbot.utils;

import com.vdurmont.emoji.EmojiParser;

public class DiscordDefaults {

    public static String roleAtMember = "662003511801675786";
    public static String roleAdmin = "723490876898148352";
    public static String roleSupport = "921420870449242162";
    public static String categorySupport = "852878611378995230";
    public static String channelWelcome = "611985124057284621";

}
